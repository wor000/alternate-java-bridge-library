...
public final class Image extends AndroidViewComponent implements OnResumeListener, OnStopListener {
  ...
  public void Picture(String path) {
    ...
    if (resourceId!=-1) {
    	ViewUtil.setBackgroundImage((android.widget.Spinner) container.$form().findViewById(resourceId), drawable);
    } else {
  ...
  private void setAnimBackground() {
	  ...
	  if (picList.size()>0) {
		  ...
		  if (resourceId!=-1) {
			  ViewUtil.setBackgroundImage((android.widget.Spinner) container.$form().findViewById(resourceId), animBackground.getFrame(0));
		  } else {
  ...
  public void setFrame(int frame) {
	  ...
	  if (frame < animBackground.getNumberOfFrames()) {
		  if (resourceId!=-1) {
			  ViewUtil.setBackgroundImage((android.widget.Spinner) container.$form().findViewById(resourceId), animBackground.getFrame(frame));
		  } else {
  ...
  public void Animation(String animation) {
	  if (resourceId!=-1) {
		  AnimationUtil.ApplyAnimation((android.widget.Spinner) container.$form().findViewById(resourceId), animation);
	  } else {
