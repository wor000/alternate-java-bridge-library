...
public class ImageSprite extends Sprite implements OnStopListener, OnResumeListener {
  ...
  public ImageSprite(ComponentContainer container, int resourceId) {
	    ...
	    ImageView bkgrd = (ImageView) form.findViewById(resourceId);
	    if (bkgrd != null) {
   ...
   */
	...
	public void setCustomAnimFrames(int[] frames) {
		...
		if (frames.length>1) {
			...
			Toast.makeText(form.$context(), "Need more than one frame to animate!", Toast.LENGTH_LONG).show();
		}		
	}
  ...
  public void gotoFrame(int frame) {
	  ...
	  if (sheetAnimation) {
		  frame--;
		  if (frame>= frameCount) {
			  Toast.makeText(form.$context(), "Frame doesn't exist!", Toast.LENGTH_LONG).show();
		  } else {
			  currentFrame = frame;
  ...
  public void Picture(String path) {
	  ...
	  */  
	  ...
	  try {
		  //drawable = form.getResources().getDrawable(temp);
		  drawable = MediaUtil.getDrawable(form, path);
	  } catch (IOException e) {
